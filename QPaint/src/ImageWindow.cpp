...
void ImageWindow::blur(){
	...
	imageLabel->setPixmap(QPixmap::fromImage(Image::Convert::convertToQt(*wImage)));
}
...
class NegateObj {
	public:
		...
		static __forceinline void process(PixelDataType * const dstImageDataPtr,PixelDataType * const srcImageDataPtr){
			(*dstImageDataPtr) = 1.0 - (*srcImageDataPtr);
		}
...
void ImageWindow::negative(){
	...
	Algorithm::BaseAlgorithm3x3<NegateObj,Image::ImageBase::DefaultPixelType::DataType>(wImage->getView(),tempImage1.getView());
...
void ImageWindow::open(){
	...
	if(image.isNull()==true) {QMessageBox::information(this, tr("Image Viewer"),tr("Cannot load %1.").arg(fileName)); return;}

	if(wImage!=nullptr) {delete wImage; wImage = nullptr;}
	wImage = new Image::Image<PixelType>(Image::Convert::convertFromQt<PixelType>(image,dataManager));
...
void ImageWindow::save(){
	...
	imageWriter.setFileName(filePathName);
	QPixmap pixmap = *imageLabel->pixmap();
	QImage image = pixmap.toImage();
	if (imageWriter.canWrite() == true){imageWriter.write(image);}
	else {QMessageBox::information(this, tr("Image Viewer"),tr("Cannot write %1.").arg(filePathName));return;}
}
