...
> class AIL_DLL_EXPORT ImageView {
	public:
		...
		ImageView(PixelDataType * const &_imageDataPtr,
				  const PixelDataType * const &_imageDataPtrEnd,
				  const ImageSize &_size,
				  const long &_numPixelsBetweenRows,
				  const bool &_hasTop,
		...
		PixelDataType &getPixel(const long &x,const long &y) {
			long tempX=x;
			if(x<0){tempX=0;
			}else if(x>=size.getWidth()){tempX=size.getWidth_1();
			}
			long tempY=y;
			if(y<0){tempY=0;
			}else if(y>=size.getHeight()){tempY=size.getHeight_1();
			}
			return imageDataPtr[tempX+tempY*size.getWidth()];
		}
		const PixelDataType &getPixel(const long &x,const long &y) const {
			long tempX=x;
			if(x<0){tempX=0;
			}else if(x>=size.getWidth()){tempX=size.getWidth_1();
			}
